 ...
 * b_name 이라는 변수에 문자열을 저장하도록 코드가 작동 될것이다
 *
 * 하지만 react 에서는 b_name = 이름 과 같은 코드는 금지된다
 * 때문에 react로 form 을 만들고 value 속성에 변수명을 지정하는 순간
 * input box 는 read only 가 된다
 *
 * 그렇다면 input에 뭔가를 입력하고 싶은데
 * read only가 되버리면 input 을 사용하는 의미가 없어진다
 * react에서는 input box에 change event를 설정하고
 * 키보드로 입력된 문자열을 가로채서 state() 함수를 사용하여
 * 값을 변화시켜야 한다.
 */
...
const Write = () => {
   ...
   * 키보드로 입력된 문자열은 e.target.value 에 담겨서 전달이 된다
   * 전달된 키보드문자열은 b_writer_text 변수에 임시로 담고
  ...
  const onChangeHandle = (e) => {
     ...
     * name 변수에 담긴 값과 같은 요소의 value값을 변경하여
     * 원래의 bbsVO와 교체하라
     *
   ...
   * 2. 현재(wirte.js) 에서 선언된 state 객체를
   *    bbs라는 이름의 변수에 담아서
   *    BBsView 에게 전달하라
   */
